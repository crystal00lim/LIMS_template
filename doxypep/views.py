                        ...
                        'children': [
                            {
                                ...
                                'children': [
                                    ...
                                    {"name": f"Need WGS<br>{data_source[s]['kb'] - data_source[s]['seq']}"}
                                ]
                            },
                            {
                                'name': f"Need KB<br>{data_source[s]['pos_sa'] - data_source[s]['kb']}"
                            }
                        ]
                    }, 
            ...
            children_branches.append({
                ...
                "children": [
                        ...
                        "children": [
                            {
                                ...
                                "children": [
                                    {"name": f"WGS<br>{r_data['wgs_sa']}"},
                                    {"name": f"Need WGS<br>{r_data['kb'] - r_data['wgs_sa']}"}
                                ]
                            },
                            {"name": f"Need KB<br>{r_data['pos_sa'] - r_data['kb']}"}
                        ]
                    },
                    {"name": f"SA Neg<br>{r_data['neg_sa']}"},
                    {
                        ...
                        "children": [
                            {"name": f"WGS<br>{r_data['wgs_e']}"},
                            {"name": f"Need WGS<br>{r_data['pos_e'] - r_data['wgs_e']}"} 
                        ]
                    },
...
def get_organism():
    ...
    orgs = {
        'label': list(trust_org.keys()),
        'series': list(trust_org.values()),
        'title': 'Organism Breakdown'
    }
...
def get_sequence_type():
    seq = WGS.objects.filter(need_resequencing=False).filter(organism__iexact='Staphylococcus aureus').values('sequence_type').annotate(count=Count('id'))
    # print(seq)
    ...
    for item in seq:
        title = 'series'
        series = {
            'x': item['sequence_type'],
            'y': item['count']
        }
        if title not in query:
            query[title] = []

        query[title].append(series)

    query.update({
        'title': 'Sequence Type Breakdown in S. aureus'
    })
